...
/**
 * Utility class for handling pagination operations. Provides methods to convert Spring Data Page
 * objects to PageResponse DTOs.
 */
public class PageUtils {
    ...
    /**
     * Creates a PageResponse from a list of items, total count, pageable and mapping function.
     * Useful for scenarios where you need to manually handle pagination logic.
     *
     ...
     */
    public static <T, R> PageResponse<R> createPageResponse(final List<T> content,
                                                            final long totalElements, final Pageable pageable, final Function<T, R> mapper) {
        ...
        final List<R> mappedContent = CollectionUtils.emptyIfNull(content).stream()
                .filter(Objects::nonNull).map(mapper).toList();
        ...
        return PageResponse.<R>builder().content(mappedContent).page(pageable.getPageNumber())
                ...
                .last(pageable.getPageNumber() >= totalPages - 1).build();
    }
     ...
     */
    public static <R> PageResponse<R> createPageResponse(final Page<R> page,
                                                         final Pageable pageable) {
        Objects.requireNonNull(page, "Page must not be null");
        Objects.requireNonNull(pageable, PAGEABLE_MUST_NOT_BE_NULL);

        return PageResponse.<R>builder().content(page.getContent()).page(pageable.getPageNumber())
                .size(pageable.getPageSize()).totalElements(page.getTotalElements())
     ...
     */
    public static <T, R> PageResponse<R> createPageResponse(final Page<T> page,
        ...
        return PageResponse.<R>builder().content(content).page(page.getNumber())
    ...
    public static <R> PageResponse<R> emptyPageResponse(final Pageable pageable) {
        ...
        return PageResponse.<R>builder().content(List.of()).page(pageable.getPageNumber())
                .size(pageable.getPageSize()).totalElements(0L).totalPages(0).first(true).last(true)
                .build();
    }
}
